...
/** Holds the primary data used on this page: metadata about hercules ontology Evolution proposals. */
var proposals
...
var REPO_PROPOSALS_BASE_URL = GITHUB_BASE_URL + 'weso/hercules-ontology/blob/master/proposals'
...
function init () {
  ...
  req.open('get', 'http://156.35.94.148:8080/api/ontology/proposals')
  req.send()
}
...
function renderBody () {
  ...
  proposalPresentationOrder.map(function (state) {
    ...
    matchingProposals.map(function (proposal) {
      ...
      if (proposal.type) detailNodes.push(renderType(proposal.type))

      // TODO: uncomment this line in the future when we add a version to the status in hercules ontology proposals
      //if (state === '.implemented') detailNodes.push(renderVersion(proposal.status.version))

      if (proposal.labels) {
        proposal.labels.forEach(label => detailNodes.push(renderLabel(label)))
      }
...
function renderLabel(label) {
  if (!label.locale || !label.label) return

  return html('div', { className: 'label proposal-detail' }, [
    html('div', { className: 'proposal-detail-label' }, `${label.locale}: `),
    html('div', { className: 'proposal-detail-value' }, label.label)
  ])
}
...
function renderType(type) {
  return html('div', { className: 'type proposal-detail' }, [
    html('div', { className: 'proposal-detail-label' }, 'Type: '),
    html('div', { className: 'proposal-detail-value' }, type)
  ])
}
...
function addEventListeners () {
  ...
  implementedToggle.addEventListener('change', function () {
    ...
    ;[].concat.apply([], expandableArea.querySelectorAll('.filter-by-hercules-ontology-version')).forEach(function (versionCheckbox) {
 ...
 * hercules ontology Evolution process.
 */
...
function _applyFilter (matchingProposals) {
  ...
  if (selectedStates.length) {
    ...
    if (selectedStates.some(function (state) { return state.match(/hercules-ontology/i) })) {
      matchingProposals = matchingProposals
        .filter(function (proposal) {
          return selectedStates.some(function (state) {
 ...
 * - version: A comma-separated list of hercules ontology version numbers to apply as a filter.
...
function _applyFragment (fragment) {
  ...
  if (actions.version.length) {
    var versionSelections = actions.version.map(function (version) {
      return document.querySelector('#filter-by-hercules-ontology-' + _idSafeName(version))
...
function _updateURIFragment () {
  ...
  var selectedVersions = document.querySelectorAll('.filter-by-hercules-ontology-version:checked')
  var versions = [].map.call(selectedVersions, function (checkbox) {
    return checkbox.value.split('hercules-ontology-hercules-ontology-')[1].split('-').join('.')
  })
...
/** Helper to give versions like 3.0.1 an okay ID to use in a DOM element. (hercules ontology-3-0-1) */
function _idSafeName (name) {
  return 'hercules-ontology-' + name.replace(/\./g, '-')
}
